...
INSERT INTO PRODUCT VALUES(12,'Train Your Brain','Written by Shireen Stephen',1);
...
SELECT COUNT(CUSTOMER.CUS_ID) AS TOTAL_CUSTOMERS,CUSTOMER.CUS_GENDER AS GENDER
FROM CUSTOMER
WHERE CUSTOMER.CUS_ID IN (SELECT `ORDER`.CUS_ID FROM `ORDER` WHERE `ORDER`.ORD_AMOUNT >= 3000)
GROUP BY CUSTOMER.CUS_GENDER;

-- 4.Display all the orders along with product name ordered by a customer having Customer_Id = 2.

SELECT `ORDER`.*,A1.PRO_NAME 
FROM `ORDER`
INNER JOIN
	(SELECT PRODUCT.PRO_NAME,SUPPLIER_PRICING.PRICING_ID
	FROM PRODUCT
	INNER JOIN SUPPLIER_PRICING
	ON PRODUCT.PRO_ID = SUPPLIER_PRICING.PRO_ID) AS A1
ON `ORDER`.PRICING_ID = A1.PRICING_ID WHERE `ORDER`.CUS_ID = 2;
...
SELECT CATEGORY.CAT_ID,CATEGORY.CAT_NAME,T2.*,MIN(T2.MIN_PRICE) 
FROM CATEGORY
INNER JOIN
	(SELECT PRODUCT.PRO_NAME,PRODUCT.CAT_ID,T1.*
	FROM PRODUCT 
	INNER JOIN
		(SELECT PRO_ID,MIN(SUPP_PRICE) AS MIN_PRICE FROM SUPPLIER_PRICING GROUP BY PRO_ID) AS T1
	ON T1.PRO_ID = PRODUCT.PRO_ID) AS T2
ON  T2.CAT_ID = CATEGORY.CAT_ID
...
SELECT CUS_NAME , IF(CUS_GENDER = 'M','MALE','FEMALE') AS GENDER FROM CUSTOMER WHERE CUS_NAME LIKE 'A%' OR CUS_NAME LIKE '%A';
